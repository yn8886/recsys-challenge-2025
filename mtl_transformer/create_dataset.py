...
def filter_by_dt_range(df, start_datetime, end_datetime):
    df = df.filter(
        (start_datetime < pl.col("timestamp")) & (pl.col("timestamp") < end_datetime)
    )
    return df
...
def get_category_propensity_target(
        client_id: int,
        propensity_category: np.ndarray,
        df: pl.DataFrame,
):
    """计算品类倾向性目标值"""
    try:
        target = np.zeros(len(propensity_category), dtype=np.float32)
        if len(propensity_category) == 0:
            return np.zeros(1, dtype=np.float32)

        cats_in_target = df.loc[df["client_id"] == client_id]["category"].unique()

        if len(cats_in_target) > 0:
            valid_indices = np.isin(propensity_category, cats_in_target, assume_unique=True)
...
def get_product_propensity_target(
        client_id: int,
        propensity_sku: np.ndarray,
        df: pl.DataFrame,
):
    """计算产品倾向性目标值"""
    try:
        target = np.zeros(len(propensity_sku), dtype=np.float32)
        if len(propensity_sku) == 0:
            return np.zeros(1, dtype=np.float32)
